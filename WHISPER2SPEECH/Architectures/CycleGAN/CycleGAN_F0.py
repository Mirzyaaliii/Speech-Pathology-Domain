...
mainfolder = "/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/old-cycleGAN/result/"+sys.argv[1] +"/F0/model/"

# Training Data path
traindata = speech_data(folder_path="/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/data/"+sys.argv[1]+"/WHSP2SPCH_F0/batches/Training_complementary_feats")
train_dataloader = DataLoader(dataset=traindata, batch_size=1, shuffle=True, num_workers=2)

# Path for validation data
valdata = speech_data(folder_path="/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/data/"+sys.argv[1]+"/WHSP2SPCH_F0/batches/Validation_complementary_feats")
...
# Initialization
Gnet_ws = generator(in_g, 1, 512, 512, 512).cuda()
Gnet_sw = generator(1, out_g, 512, 512, 512).cuda()
Dnet_w = discriminator(in_d, out_d, 512, 512, 512).cuda()
Dnet_s = discriminator(1, out_d, 512, 512, 512).cuda()


# Optimizers
optimizer_G = torch.optim.Adam(itertools.chain(Gnet_ws.parameters(), Gnet_sw.parameters()),lr=learning_rate)
optimizer_D_w = torch.optim.Adam(Dnet_w.parameters(), lr=learning_rate)
optimizer_D_s = torch.optim.Adam(Dnet_s.parameters(), lr=learning_rate)
...
def training(data_loader, n_epochs):
    ...
    for en, (a, b) in enumerate(data_loader):
        ...
        # GAN loss
        Gout_ws = Gnet_ws(a)
        loss_GAN_W2S = criterion_GAN(Dnet_s(Gout_ws), valid)*10
        
        Gout_sw = Gnet_sw(b)
        loss_GAN_S2W = criterion_GAN(Dnet_w(Gout_sw), valid)*10
        
        # Cycle loss
        recovered_W = Gnet_sw(Gout_ws)
        loss_cycle_WSW = criterion_cycle(recovered_W, a)*1
        
        recovered_S = Gnet_ws(Gout_sw)
        loss_cycle_SWS = criterion_cycle(recovered_S, b)*1
...
#        optimizer_G.step()
        ...
        # Total loss
        loss_D_w = (loss_D_real + loss_D_fake)*2
        loss_D_w.backward()
        ...
        # Total loss
        loss_D_s = (loss_D_real + loss_D_fake)*2
        loss_D_s.backward()
...
if isTrain:
    ...
    for ep in range(epoch):
        ...
        gl_arr.append(gl)
        if ep == 0:
            gplot = viz.line(Y=np.array([gl]), X=np.array([ep]), opts=dict(title='Cycle_Generator_F0'))
            dplot = viz.line(Y=np.array([dl]), X=np.array([ep]), opts=dict(title='Cycle_Discriminator_F0'))
        else:
...
# -------------------- Testing -----------------------------------------------------------------------------------------------------------------------------
    ...
    print("Testing")
    save_folder = "/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/old-cycleGAN/result/"+sys.argv[1] +"/F0/mask/"
    test_folder_path="/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/old-cycleGAN/result/"+sys.argv[1]+"/MCEP/mask/"

...
class speech_data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        id1 = np.random.randint(len(dd1))
        id2 = np.random.randint(len(dd2))
        # print(len(dd1),id1,len(dd2),id2)
        d1 = loadmat(join(fd1,dd1[id1]))
        d2 = loadmat(join(fd2,dd2[id2]))

        return np.array(d1['Clean_cent']), np.array(d1['Feat']),np.array(d2['Clean_cent']), np.array(d2['Feat']),self.c_org,self.c_trg
...
mainfolder = "/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/star-gan/result/model_f0/"

traindata = speech_data(folder_path="/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/data", mainfolder="WHSP2SPCH_F0/batches/Training_complementary_feats")
train_dataloader = DataLoader(dataset=traindata, batch_size=1, shuffle=True, num_workers=1)
...
Gnetf = generator(40, 1,no_of_domains, 512, 512, 512).cuda()
Gnetn = generator(1, 40,no_of_domains, 512, 512, 512).cuda()
Dnet = discriminator(1, 1,no_of_domains , 512, 512, 512).cuda()
...
# Optimizers
optimizer_G = torch.optim.Adam(itertools.chain(Gnetn.parameters(), Gnetf.parameters()), lr=0.0001)
...
def training(data_loader, n_epochs):
    ...
    for en, (b_org, a_org,b_trg,a_trg,c_org_index,c_trg_index) in enumerate(data_loader):
        ...
        Gout_c_trg = Gnetf(a_org,c_trg)
        G_loss_adv_t = adversarial_loss(Dnet(Gout_c_trg)[0], valid)			#adversial loss
        G_loss_adv_s = adversarial_loss(Dnet(b_org)[0], fake)

        G_loss_cyc = mmse_loss(Gnetn(Gout_c_trg,c_org),a_org)*1	#cycle loss  change =  b_org->a_org		
		
        G_loss_id = mmse_loss(Gnetf(a_org,c_org),b_org)*1 + mmse_loss(Gnetn(b_trg,c_trg),a_trg)*1		#identity loss
...
if isTrain:
    ...
    for ep in range(epoch):
        ...
        if (ep+1)%5==0:
            torch.save(Gnetf, join(mainfolder,"gen_gf_{}_d_{}_Ep_{}.pth".format(5,1,ep+1)))
            torch.save(Gnetn, join(mainfolder,"gen_gn_{}_d_{}_Ep_{}.pth".format(5,1,ep+1)))
...
# -------------------- Testing -----------------------------------------------------------------------------------------------------------------------------
    ...
    print("Testing")
    save_folder = "/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/star-gan/result/US_106/F0/mask/"
    test_folder_path="/media/maitreya/Maitreya_Hard_Disk/Speech_Lab_VC/INTERSPEECH19-paper2/star-gan/result/US_106/MCEP/mask/"
    n = len(listdir(test_folder_path))
    Gnet = torch.load(join(mainfolder,"gen_gf_5_d_1_Ep_100.pth"))
    for i in range(n):
